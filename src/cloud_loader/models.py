...
class Concept(SQLModel, table=True):
    """Concept tracking configuration."""

    __tablename__ = "concepts"

    id: Optional[int] = Field(default=None, primary_key=True)
    user_id: str = Field(index=True, max_length=20)
    name: str = Field(max_length=100)
    description: str = Field(max_length=500)
    keywords: str = Field(default="[]")  # JSON list of keywords
...
class ConceptSnapshot(SQLModel, table=True):
    """Snapshot generated by an AI run for a concept."""

    __tablename__ = "concept_snapshots"

    id: Optional[int] = Field(default=None, primary_key=True)
    concept_id: int = Field(foreign_key="concepts.id", index=True)
    knowledge_graph: str = Field(default='{"nodes": [], "edges": []}')  # JSON
    summary: str = Field(default="")
    source_urls: str = Field(default="[]")  # JSON array
    content_drafts: str = Field(default="{}")  # JSON object
    created_at: datetime = Field(default_factory=_utc_now)
